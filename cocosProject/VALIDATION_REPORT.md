...
| Asset metadata | Pass | 100 asset files，missing meta `0`；`BagLikeHealing.ts` 已由现有 Creator 正规生成 meta；H0905 效果资源、既有原始 UI、shape Spine 与融合英雄 Spine 保持可加载 |
...
| Weighted preparation draw | Pass (supported-family runtime path) | `40/40`：静态资格、3000–3004 路由、独立槽位抽取、账号已解锁族过滤、H11 账号锁定例外、金币计数权重、五英雄族上限/缺失族替换、第七次非广告刷新强制扩格、Prepare 精确倍率及原版 3012 no-op；仍缺目标账号精确解锁集合 |
| H11 healing gear | Pass (numeric/runtime linkage), matched effects pending | `43/43`：H1101–H1104 等级倍率、ZL_1101 的 0.25 秒一次性 WHEEL 完成链、半径 200 内最低生命比例存活友军、先单位 100% 攻击治疗再基地 50% 攻击治疗、floor/clamp 及无目标边界；账号星 2/5/7 扩展因目标值未知而保持关闭 |
...
| Refresh | 自动准备发牌、首次普通刷新免费、后续 15 金币、每准备回合一次广告刷新、刷新替换未摆放候选；静态资格与 3000–3004 基础权重树；账号解锁族过滤与 H11 账号锁定例外；金币齿轮计数驱动 3034 权重；最多五种英雄族及缺失族替换；每七次非广告刷新强制扩格；二级概率能力保留 3012 配置断链 no-op | 目标账号精确解锁集合 |
...
| Production | 核心逐侧触发连通块、100 点工人进度、HAMSTER 0.75 秒输出延迟、H12/H13 一次性技能、H11 0.25 秒一次性治疗链、银币完成产出、1.5 倍速；核心直接相邻齿轮可按能力对后续 HAMSTER/WHEEL 攻击乘 1.2，或对每次工人进度及 `/s` 显示乘 1.2，且不会误作用于仅连通齿轮、候选区或 HP | 首次核心接触相位和同帧顺序仍待 matched trace |
