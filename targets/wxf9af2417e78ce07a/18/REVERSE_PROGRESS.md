...
- H12 的 20 是单次进度，满后从齿轮位置释放一次随机目标、半攻倍率、半径 50 内最多 5 目标的 WHEEL 技能，并不生成常驻 H12 单位；M03/Boss03 远程攻击为半攻倍率。
...
- 已纠正“截图三张能力固定轮换”的错误：原版每次使用 `getChooseBuff(3)` 按权重抽取，过滤
  次数上限、阵容范围、账号星级、条件/验证；当前实现池中的 28 条原表效果行按 25 个互斥
  能力组参与抽取（24 个有效组和 1 个版本配置断链的确认无效项），并走
...
- **准备动态发牌规则（2026-08-02）**：`BagLilkeManager.refreshBrick`、
  ...
  发牌路径并有 `40/40` 确定性断言。H11 的治疗消费者完成后，原版账号锁定过滤例外也已在
  正式候选路径启用。证据见 `evidence/runtime/baglike-preparation-dynamic-rewards.md`。
- **H11 治疗齿轮（2026-08-06）**：`H1101..H1104 → ZL_1101 → B_ZL_1101 / B_ZL_1102`
  已按版本 18 运行时接入。四级工人进度均为 `9`，属性倍率为
  `1 / 1.5 / 2.25 / 3.375`；进度完成后等待 `0.25s`，在半径 `200` 内选择生命比例最低的
  存活友军，先恢复 `floor(攻击力 × 1.0)`，再为我方基地恢复
  `floor(攻击力 × 0.5)`，两者均封顶。没有存活友军时不执行该次治疗链。账号星级驱动的
  星 2/5/7 扩展因目标账号值缺失而保持关闭。纯 TypeScript 消费者有 `43/43` 断言；连同
  候选例外在内，全量规则/资源测试为 `564/564`。证据见
  `evidence/runtime/baglike-h11-healing-gear.md`。

## 当前非阻塞缺口

- 需要在 Creator 中完成一次完整点击通关冒烟测试。
- 至少约 30 条当前已表现英雄族相关 BagLike 效果仍未接入真实运行时消费者；玩家账号解锁、
...
- 规划估算：1004 机制/数据约 `70%–75%`，全约 200 关系统约 `45%–55%`；正式门禁仍以
  `BATTLEFIELD_RESTORE_STATE.json` 为准，当前四门禁均未通过。
- 自动基线为 golden `47/47`、规则/资源 `564/564`、动态发牌 `40/40`、H11 治疗
  `43/43` 和 Creator TypeScript/Cocos 项目检查通过。
