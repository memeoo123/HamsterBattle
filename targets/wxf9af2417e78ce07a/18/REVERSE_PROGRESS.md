...
- 核心首次完成四分之一圈接触方向、准备到战斗的连续相位、战斗控制器同帧顺序、出生位置原生随机与战斗种子 RNG 的调用域均已恢复；终局弹丸/EXP/奖励顺序与失败断点生命周期也已由源码关闭。公共计时器、启动器与 Cocos Scheduler 源码已关闭最后的跨调度器边界：同一引擎 tick 中 `GameTimer` 核心回调先于 `BattleTimer` 战斗帧。
