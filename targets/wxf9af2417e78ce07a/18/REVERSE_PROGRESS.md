...
- **准备动态发牌规则（2026-08-02）**：`BagLilkeManager.refreshBrick`、
  ...
  发牌路径并有 `38/38` 确定性断言。H11 是原版账号锁定过滤例外，但需先恢复其治疗消费者再
  正式启用。证据见 `evidence/runtime/baglike-preparation-dynamic-rewards.md`。

## 当前非阻塞缺口

- 需要在 Creator 中完成一次完整点击通关冒烟测试。
- 下一机制切片为 H11 治疗齿轮完整消费者；完成前不启用其账号锁定过滤例外。
- 至少约 27 条当前已表现英雄族相关 BagLike 效果仍未接入真实运行时消费者；玩家账号解锁、
...
- 自动基线为 golden `47/47`、规则/资源 `519/519`、动态发牌 `38/38` 和 Creator TypeScript 通过。
