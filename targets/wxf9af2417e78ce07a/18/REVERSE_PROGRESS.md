...
- 已把版本化账号成长、关卡奖励/推进与本机 200 关运行时合并；账号专项、候选连接、主场景和完整关卡状态机共享同一正式消费者。
- 目标竞品账号的真实资源、星级与碎片值仍不在包体内；当前结论证明规则和本地闭环，不声称复现同一个玩家存档。
...
- **P01 动力仓鼠旋转与拖动（2026-08-12，2026-08-13 阶段/转速修订）**：保留 `BrickShowBaseCom`、通用拖动控制器和 FairyGUI `power1.png` 的资源与生产计时证据；用户对竞品的直接观察进一步确认中心齿轮准备态静止、战斗态才启动，并指出旧视觉转速过快。重建现于每波战斗从 0° 启动，中心视觉降为 4 秒一圈，而原表 1 秒生产整圈/200 ms 接触延迟不变。1004 实测准备态 `0.0° → 0.0°`，战斗态约一秒采样 `46.5° → 138.1° → 231.2°`，warning/error 0；专项 51、全量 54/54、Golden 47/47、TypeScript 和新鲜 Web Mobile 构建通过。证据见 `evidence/runtime/p01-battle-only-motion-2026-08-13/manifest.json`。
...
- **1100 十五波正式闭环（2026-08-09）**：旧 10% 长期补偿下限探索运行 25 次失败仍未可靠收敛；保留恢复原表与前 3 次精确补偿，仅将重建专用长期可达性下限降至 1%。新 Web 构建从零累计到第 17 次后，通过正常拖放、连通块布阵、合成、特质和战斗完成全部 15 波并进入 `won`，失败计数清零；获胜段峰值己方 9、敌方 22，0 运行错误、0 缺失生产配置。证据见 `evidence/runtime/level-1100-15-wave-closure-current/`。

- 关内机制数据门禁已通过；战斗帧顺序、核心连续相位、同帧刷怪/弹丸/销毁，以及原生坐标随机与 `9301/49297/233280` 种子战斗 RNG 分域均已接入并具备确定性测试。
- 新 Web 构建已真实完成候选拖放、开战、三侧连通生产、失败和重新挑战；临时验证服务已关闭，未发现项目控制台错误。
- 修复 P01 生产加速与 Creator Release 的 Set 展开后，默认重建账号已通过 1002 全部 8 波；机制优先补偿在第 6 次挑战收敛，完整胜利门禁现有真实交互证据。
- 终局源码边界已关闭：到期弹丸先结算、基地死亡优先，最后击杀同步给 EXP 并立即清除未来弹丸，回合奖励走 1000 ms 回调；未完成战斗 `failedTimes` 可进入断点，但失败/结束/退出会清除记录。公共包源码进一步确认两个 Timer 实际均以 `M` 注册，`BaseLauncher` 先创建 `GameTimer`，Cocos 同优先级尾插顺序稳定，所以同 tick 固定为核心生产先、战斗帧后；复原运行时及第 32 条生产契约与此一致。代表账号成长输入保留为原版平衡高保真复现项。
- Boss03 的 `BT_001` 控制免疫已接入实际可选能力验证：H12 `LY_1202/LY_1203` 麻痹和 H03 星 7 眩晕均读取同一 live `controlImmune` 标志，Boss03 仍承受技能伤害和变形表现时长，但不进入控制状态；专项 18/18 通过。
- 总编排保持 `validation / in_progress`，当前登记的主阻塞仍为 `check:visualBaseline`；本轮工作区改动尚未暂存、提交或推送。
...
- 核心在战斗开始后首次完成四分之一圈接触方向、战斗控制器同帧顺序、出生位置原生随机与战斗种子 RNG 的调用域均已恢复；准备态相位已按 2026-08-13 竞品观察修订为静止。终局弹丸/EXP/奖励顺序与失败断点生命周期也已由源码关闭；同一战斗 tick 中 `GameTimer` 核心回调先于 `BattleTimer` 战斗帧。
...
- 自动验证为当前 golden 文件 `47/47`、13 个机制脚本 `885/885`、全量 30 脚本 `5943/5943`、特性池 `240/240`、战斗内核 `116/116`、生产时钟 `31/31`、Creator TypeScript 上一基线通过，
...
## 战斗帧、动力核心与 RNG 顺序切片（2026-08-07）

- 原版战斗帧现已固定为：到时刷怪 → 队伍/碰撞快照 → 英雄分离 → Buff/战斗计时器 → 英雄逆序 → 怪物逆序 → 领袖技能逆序 → 延迟销毁/深度排序 → 弹丸逆序 → 自动战斗 → 胜负检查。到时生成的怪物同帧参与行动，单位行动中新建的弹丸同帧进入弹丸更新，单位延迟销毁先于弹丸。
- 2026-08-13 按用户对竞品阶段行为的直接观察修订：准备阶段动力核心视觉与生产时钟均静止；每波开战时归零并从方向 1 启动。原 `P01.params=[1000,200,10000]` 继续限定生产整圈与接触延迟，中心视觉独立使用 4 秒近似整圈，等待竞品录像逐帧关闭精确转速。
- 战斗随机现与坐标随机拆分。出生 Y、工厂 X/Y 抖动保持原生 `Math.random`；战斗决策使用 `BattleManager` LCG，默认种子 5，递推为 `(9301 * seed + 49297) % 233280`。模数由实际启用的混淆分支求和得到，不是经验猜测。
- 证据见 `evidence/runtime/battlefield-frame-rng-order.md`。13 个机制脚本全量 `885/885`、项目全量 30 脚本 `5943/5943` 通过；战场四门禁仍未整体关闭，下一机制任务是完整胜利集成闭环。
- 新 Creator 3.8.8 Web 构建已完成；真实交互已覆盖 1002 候选拖放、两次合成、H04 替换、P01 加速、开战、特质、5 次失败/重新挑战和全部 8 波。Release Set 展开修复后最终会话 5,916 次生产应用无丢失，第 6 次挑战以 361/1375 HP 无改值胜利。代表账号 P01/英雄进度输入继续作为原版账号高保真取证项；下一机制闭环目标为 15 波中/晚关。

## H1005 type-11 普攻弹体切片（2026-08-09）

- 已纠正早期把 `type=11` 推断为 Ray 的结论：v18 `BulletType.Ray=18`，工厂没有
  value 11 分支，因此 `M_FD_10001` 实际进入默认 `BulletUnit`。
- H1005 现从 `SkillConfig 10001` 的 `atkPoint=(0,50)` 发射，快照初始目标方向，按
  speed 1000 飞到目标初始点前约 20 像素；到点后从弹体位置朝仍存活的锁定目标结算一次
  5000 比率、100×300 的 `SELF_RECTANGLE`。目标提前死亡时不重选目标、不补伤害。
- `M_FD_10001` 的 `distance=500`、`timeLimit=4000`、`parameter.width=600`、
  `interval=1000`、`autoLock=600` 在默认弹体路径中均无读取点，已确认是断接字段而非待补机制。
