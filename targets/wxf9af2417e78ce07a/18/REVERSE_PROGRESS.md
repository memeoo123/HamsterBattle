...
## 账号/英雄星级扩展（2026-08-10）

- 原 `HeroModel` 的账号链已落实到恢复工程：`HeroLocalVo.stars`、0 星锁定、1–20 星、逐关挑战次数和损坏存档回退均进入版本化本地档案；准备界面提供可操作入口。
- 完整 20 星 `attrModifier` 与 `floor(base×(1+modifier/10000))` 顺序已接入。账号星级现在同时影响动态候选、特质、五级融合、HAMSTER ATK/HP、WHEEL ATK 与基地 HP；重试会推进当前关卡挑战次数。
- 新增专项 `42/42`，候选掉落/渲染连接升至 `41/41`，全量变为 27 脚本 `1082/1082`；Creator 3.8.8 检查为 170 资源、0 missing meta、TypeScript 通过，Web Mobile 构建任务完成，750×1334 账号面板运行时无项目控制台错误。
- 这完成的是可持久化账号系统，不代表包体包含目标玩家的真实存档。除 H13 3 星可由 863 HP 截图独立反推外，其余精确目标账号值仍是外部证据边界。
...
| Creator 工程校验 | 自动校验与失败/重试交互通过 / 15 波胜利待验 | 13 个脚本 `882/882`；Creator 3.8.8 Web 构建完成且浏览器无项目控制台错误；仍需有证据账号输入下的完整 15 波胜利闭环 |
...
- 当前证据默认账号/挑战配置即使使用三侧 H13/H01/H12 连通组件仍在第 1 波失败，因此没有把 15 波胜利门禁误标为通过。
- 下一机制任务：恢复代表账号的英雄星级、挑战次数和起始发展输入，再完成第 1–15 波胜利；同时继续核对核心 `GameTimer` 与战斗 `BattleTimer` 同 tick，以及同时死亡、待结算弹丸、奖励/EXP 的终局边界。
...
- 核心首次完成四分之一圈接触方向、准备到战斗的连续相位、战斗控制器同帧顺序、出生位置原生随机与战斗种子 RNG 的调用域均已恢复；仅 `GameTimer` 核心回调与 `BattleTimer` 战斗帧在同一引擎 tick 的跨调度器先后，以及跨进程失败次数仍需 matched trace/生命周期验证。
...
- 自动验证为当前 golden 文件 `47/47`、规则/资源测试 `882/882`、特性池 `240/240`、战斗内核 `116/116`、生产时钟 `28/28`、Creator TypeScript 上一基线通过，
...
- 动力核心 `GameTimer` 不再在开战时归零：零角度不是接触，首次完成四分之一圈接触方向 1；准备阶段继续转动，已占用侧仍产生 200 ms 停顿，但 `ShowNodeCom` 在非 BATTLE 状态不增加工人进度。
...
- 证据见 `evidence/runtime/battlefield-frame-rng-order.md`。13 个脚本全量 `882/882` 通过；战场四门禁仍未整体关闭，下一机制任务是完整 1–15 波胜利与失败/重试集成闭环。
- 新 Creator 3.8.8 Web 构建已完成并通过本地加载；真实交互已覆盖候选拖放、开战、失败和重新挑战。双侧 H13 显示 `0.17/s`，三侧连通 H13/H01/H12 分别约 `0.28/s / 0.19/s / 0.38/s`，验证了整连通块继承全部核心接触侧。当前证据默认账号/挑战配置仍在第 1 波失败，因此完整胜利闭环保持 pending；下一步必须先恢复有证据的代表账号进度/挑战输入，不能擅自削弱敌人。

## 账号资源、关卡奖励与正式升星闭环（2026-08-10）

- 已从 `HeroStarConfig` 固定 1–20 星精确消耗，并从 `HeroConfig` 固定基础解锁与
  `PASS_TRUNK_INSTANCE` 解锁：H13=1001、H03=1002、H11=1004。
- 已逐关验证 1001–1200 的三档奖励公式与 `TrunkInstanceConfig` 一致；最终档固定钻石 100，
  `BOX_RF` 数量为 `chapter*2+14`。`BOX_RF` 的 12 个英雄碎片族保持原配置等权概率。
- 恢复工程已接入 schema v2 本地账号：金币、体力、钻石、12 族碎片、最大通关关卡、星级和
  挑战次数均持久化；旧 schema 原位迁移。波次奖励、胜利通关、自动解锁和正式升星形成闭环。
- 自动验证为 `1133/1133`（27 文件），账号专项 `90/90`，候选专项 `44/44`，TypeScript
  通过；Creator Web Mobile 成功码 36，最终账号面板实机日志 warning/error 0。
- 原包仍不包含目标竞品账号的精确资源、星级和碎片存档，因此本结论证明规则与本地闭环，
  不声称复现同一个竞品账号。完整证据见 `evidence/runtime/account-progression-loop.md`。

下一实现切片：关卡选择与 1001–1200 连续推进入口；随后用规则内账号完成 1004 十五波胜利
闭环，再执行 200 关批量加载/奖励冒烟。
